...
bool Maze::validNewLocation(Position oldLoc, Position newLoc){
    if (newLoc == end){
        maze[oldLoc.getY()][end.getX()-1] = true;
        return true;
    }
    else if ( (newLoc.getX() <=0 || newLoc.getX() >=maze_size_x-2 || newLoc.getY() <=0 || newLoc.getY() >=maze_size_y-2) ){
        return false;
    }
...
Position Maze::chooseMove(Position oldPos){
    ...
    bool validNewLoc = false;
    while (tryCount <4 && !validNewLoc){
        if (direction == 0){
        ...
        validNewLoc = validNewLocation(oldPos, newLoc);
    }
    if (tryCount >=4){
        newLoc = Position(-1, -1);
    }
    else {
        morePotMoves.push_back(Position(oldPos.getX(), oldPos.getY()));
    }
    return newLoc;
}
